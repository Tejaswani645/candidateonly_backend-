...
// openedBy remembers which contacts have already opened per campaign, so repeat opens by
...
// seen tracks event IDs we have already processed across all batches.
...
func apply(ev Event) {
	...
	switch ev.Type {
	case "sent":
		atomic.AddInt64(&cs.Sent, 1)
	case "delivered":
		atomic.AddInt64(&cs.Delivered, 1)
	case "opened":
		atomic.AddInt64(&cs.Opened, 1)
	case "clicked":
		atomic.AddInt64(&cs.Clicked, 1)
	}
}
